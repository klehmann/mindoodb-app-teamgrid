...
- Rows and columns can be hidden and unhidden. Right-click a row or column header (or use Edit / Hide rows · Hide columns) to collapse it to zero size; selecting the neighbour rows or columns and choosing Unhide restores them. The hidden flag is preserved on XLSX import and export so the state survives an Excel round trip.
